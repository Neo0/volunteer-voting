...
class OperatorVotingDB
{
    ...
    {
    	$time = time();
    	$time1 =strtotime("2015-12-18 08:00:00");//投票截止时间
    	if(($time-$time1)<0)
    	{
    		// $sql = "INSERT INTO ip_votes VALUES (NULL, '$ip', '$loc', NOW(), '$val')";
	        ...
	        if (count($row=$stm->fetchAll())==1)
	        {
	            ...
	            {
	                echo '<script type="text/javascript">alert("投票失败，相同ip需要隔一天才能投票");location.href="index.php"</script>';
	                return;
	            }
	        }
	        ...
	        if ($flag==1)
	        {
	            echo '<script type="text/javascript">alert("投票成功！");location.href="index.php"</script>';
	        }        
    	}
    	else
    		echo '<script type="text/javascript">alert("投票已截止！");location.href="index.php"</script>';
